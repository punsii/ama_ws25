...
# (1) econ PCs only
rhs_socio_1 = " + ".join([str(econ_pc1), str(econ_pc2)])
socio_1 = registry.fit(
    df_train[[LECol.TARGET, econ_pc1, econ_pc2]],
...
rhs_socio_2 = " + ".join([STATUS_DUMMY, str(econ_pc1), str(econ_pc2), str(undp_pc1)])
socio_2 = registry.fit(
    df_train[[LECol.TARGET, STATUS_DUMMY, econ_pc1, econ_pc2, undp_pc1]],
...
rhs_socio_3 = " + ".join(
    [STATUS_DUMMY, str(econ_pc1), str(undp_pc1), f"I({undp_pc1} ** 2)"]
)
socio_3 = registry.fit(
    df_train[[LECol.TARGET, STATUS_DUMMY, econ_pc1, undp_pc1]],
...
].loc[["m_socio_eco_pcs", "m_socio_eco_undp", "m_socio_eco_undp_quad"]]
```
...
base_cols = [LECol.TARGET, *immun_base_block]
base_diag = registry.fit(
    df_train[base_cols],
    name="m_immun_base",
    rhs=" + ".join(str(c) for c in immun_base_block),
)
display(base_diag)

immun_cols = [LECol.TARGET, *immun_base_block, immun_pc1]
