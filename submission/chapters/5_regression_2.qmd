...
def cmp_tbl(registry: ModelRegistry, features: list[str]) -> pd.DataFrame:
    return registry.compare(sort_by="aic").loc[features].sort_values(
        "aic"
...
def make_dataset(year: int) -> pd.DataFrame:
    return (
        ...
        .tf_and_norm()
        .set_index("iso3")
        .pipe(lambda df: df[df.index.notna()])
        .drop(
            columns=[
                LECol.YEAR,
                HDRCol.LFPR_F,
...
Considering that the first principal component already captures ~89.4% of the variance in the social development block (@fig-pca-group-variance-2) consisting of six features, we retain only the first PC and treat it as a compact summary of the social-development dimension for life expectancy modeling.
:::
...
cmp_tbl(registry, ["m1_adult_mortality", "m1_adult_mortality_hdi"])
```
...
The newly added `social_development_PC1` is only marginally significant in the full training sample (p = 0.045; CI **[-1.002, -0.001]**), but it improves fit slightly (AIC 900.32 $\rightarrow$ 898.15). To assess robustness of this socioeconomic baseline, we refit the same specification after removing influential observations via Cook’s distance ($> 4/n$: **5** countries removed). We treat this trimming as a sensitivity analysis (influential $\neq$ incorrect), not as evidence of data errors.
...
comp_tbl = cmp_tbl(registry, socio_econ_names)

best_socio_econ_name = comp_tbl.index[0]
best_socio_econ = registry.get(best_socio_econ_name).diag
best_socio_econ_rhs = registry.get(best_socio_econ_name).rhs
display(comp_tbl)
```
...
The best AIC socioeconomic baseline after Cook’s-distance sensitivity trimming is
`life_expectancy ~ status_developed + economic_development_PC1 + social_development_PC1 + undp_socio_PC1 + I(undp_socio_PC1^2)` ($n=156$; `m_socio_eco_social_cook`).
On the 2014 training cross-section, it achieves **$R^2=0.861$** (adj. **0.857**) with **RMSE = 2.81 years**, showing that broad development indicators alone already explain most cross-sectional variation in life expectancy.

Developed status remains substantial (**+4.33 years**, $t=5.06$, $p<0.001$) even after the continuous development PCs. The economic-development component is positive (**+1.10 years per 1 SD**, $t=3.84$, $p<0.001$). The UNDP socioeconomic component dominates the fit and is nonlinear: the linear term is large (**-2.53**, $t=-9.41$, $p<0.001$) and the quadratic term is also negative (**-0.53**, $t=-8.70$, $p<0.001$), implying concavity (non-constant marginal association). The `social_development_PC1` effect is small (**-0.53**, $p=0.013$) and is plausibly sensitive to overlap and suppression between multiple correlated “development” summaries (moderate collinearity; max VIF $\approx 6.17$).

Diagnostics in @fig-socioeconomic-diagnostics are favorable after trimming: residual normality is not rejected (Jarque–Bera $p=0.195$; Shapiro $p=0.121$) and heteroscedasticity tests do not reject (Breusch–Pagan $p=0.087$; White $p=0.331$). Influence is bounded (max Cook’s **0.035**, max leverage **0.162**), indicating that the estimates are not dominated by a single high-leverage country. We treat Cook’s-distance trimming as a **sensitivity analysis** (influential $\neq$ incorrect) and use it primarily to improve stability and diagnostics; note that AIC values are most comparable across models fit on the same dataset and sample size.
...
Inspecting @tbl-socioeconomic-eval, that the trimmed socio-economic model, which we declared the winner on AIC grounds, has the second worst performance on the hold out set. This is an interesting observation that aligns with analogous findings in [Supervised PCA Regression](5_regression_supervised_pca.qmd), where the simplified and trimmed model did not generalize as well as the fuller models. However, since our primary goal is interpretability and stable effect estimation rather than pure prediction, we proceed with the trimmed socioeconomic baseline for subsequent analyses.

We subsequently inspect holdout calibration for the "best" socioeconomic baseline:
...
ax = eval_socio[best_socio_econ_name].plot_calibration(
    bins=10,
    bootstrap=N_BOOTSTRAP,
    random_state=42,
)
target_pretty = scaled_train_ds.get_pretty_name(str(LECol.TARGET))
ax.set_xlabel(f"Predicted {target_pretty}")
ax.set_ylabel(f"Observed {target_pretty}")
plt.show()
```

The bootstrapped CIs mirror closels the density of datapoints in the calibration plot, with wider CIs at lower predicted and ground-truth life expectancy values. Overall, the socioeconomic baseline shows reasonable calibration on the holdout year.

## Immunization impact

**RQ3:** quantify the effect of immunization coverage (e.g., DTP3, HepB, Polio) controlling for development; furthermore we explore the impact of the number of HIV cases against this baseline.

Baseline (socioeconomic): `m_socio_eco_social_cook` (RQ2)

HIV burden: `life_expectancy ~ m_socio_eco_social_cook + hiv_aids`
Immunization model: `life_expectancy ~ m_socio_eco_social_cook + immunization_PC1`

:::{.callout-note collapse="false"}

### M1: HIV Burden
```{python}
# | label: tab-hiv-burden-model
# | code-fold: true
...
SOCIO_BASE_NAME = "m_socio_eco_social_cook"
if SOCIO_BASE_NAME not in registry.models:
    raise KeyError(
        f"Expected socioeconomic baseline '{SOCIO_BASE_NAME}' missing; check RQ2 section."
    )

# Use the selected socioeconomic baseline terms from RQ2 and add mortality burden (HIV/AIDS).
socio_terms = [STATUS_DUMMY, econ_pc1, social_le_undp_pc1, undp_pc1]
socio_extra = [f"I({undp_pc1} ** 2)"]
COOK_THR = 4 / len(df_train)

hiv_initial, hiv_diag = fit_additive(
    registry,
    df_train,
    name="m_hiv_base",
    terms=[*socio_terms, LECol.HIV_AIDS],
    extra_terms=socio_extra,
    refit=True,
    cook_distance_threshold=COOK_THR,
)
registry.remove("m_hiv_base_initial")
display(hiv_diag)
```

```{python}
# | label: fig-hiv-diagnostics
# | fig-cap: "HIV burden model diagnostics."
hiv_diag.plot_residual_diags()
plt.show()
```

The HIV burden is clearly significant with a negative association (**-2.23 years per 1 SD increase in HIV cases**), p < 0.001; Adding HIV improves the model fit from and AIC of 772 to 740 while still satisfying all OLS assumptions.
:::
...
# | code-fold: true
immun_initial, immun_diag = fit_additive(
    registry,
    df_train,
    name="m_immunization",
    terms=[*socio_terms, immun_pc1],
    extra_terms=socio_extra,
    refit=True,
    cook_distance_threshold=COOK_THR,
)
registry.remove("m_immunization_initial")
display(immun_diag)
```
...
immun_diag.plot_residual_diags()
```
:::

```{python}
# | label: tbl-immunization-compare
# | tbl-cap: "HIV baseline vs immunization model (training fit), sorted by AIC."
registry.compare(sort_by="aic").drop(columns=["rhs"])[
    ["aic", "rmse", "r2", "adj_r2", "n_obs"]
].loc[["m_hiv_base", "m_immunization"]].sort_values("aic").round(3)
```

Adding `immunization_PC1` worsens fit relative to the base model by 8 AIC points. While the `immunization_PC1` negative (it represents _under-coverage_), as expected it is not statistically significant (p = 0.462), hence we cannot reject the null hypothesis of no effect.
...
M1: `life_expectancy ~ m_socio_eco_social_cook + alcohol`
M2: `life_expectancy ~ m_socio_eco_social_cook + bmi`
...
controls = socio_terms

alc_initial, alc_diag = fit_additive(
    registry,
    df_train,
    name="m_alcohol",
    terms=[*controls, LECol.ALCOHOL],
    extra_terms=socio_extra,
    cook_distance_threshold=4 / len(df_train),
)
registry.remove("m_alcohol_initial")
display(alc_diag)
```

```{python}
# | label: alcohol-diagnostics
# | fig-cap: "Alcohol model diagnostics."
...
# | label: bmi-model
# | code-fold: true
bmi_initial, bmi_diag = fit_additive(
    registry,
    df_train,
    name="m_bmi",
    terms=[*controls, LECol.BMI],
    extra_terms=socio_extra,
    cook_distance_threshold=4 / len(df_train),
)
registry.remove("m_bmi_initial")
display(bmi_diag)
```

```{python}
# | label: bmi-diagnostics
# | fig-cap: "BMI model diagnostics."
_ = bmi_diag.plot_residual_diags()
```

```{python}
# | label: fig-alc-bmi-scatter
# | fig-cap: "Scatter plots of life expectancy against alcohol consumption and BMI (with LOWESS curves)."
...
axes[0].set_title("Life Expectancy vs Alcohol Consumption")
axes[0].set_xlabel(scaled_train_ds.get_pretty_name(str(LECol.ALCOHOL)))
axes[0].set_ylabel(scaled_train_ds.get_pretty_name(str(LECol.TARGET)))
sns.regplot(
    x=LECol.BMI,
    y=LECol.TARGET,
    data=df_train,
...
axes[1].set_title("Life Expectancy vs BMI")
axes[1].set_xlabel(scaled_train_ds.get_pretty_name(str(LECol.BMI)))
axes[1].set_ylabel(scaled_train_ds.get_pretty_name(str(LECol.TARGET)))
plt.tight_layout()
plt.show()
```
:::
...
**Answer (RQ4):** conditional on the socioeconomic baseline, BMI shows a clear nonlinear association (increase at low-to-mid values with saturation at higher values), while alcohol shows only mild nonlinearity (mostly flat with an increase at the upper end).

We fit low-complexity B-splines guided by @fig-alc-bmi-scatter: alcohol uses `degree=1` with `n_knots=2` ($df=3$), and BMI uses `degree=3` with `n_knots=2` ($df=5$).

:::{.callout-note collapse="false"}
### M2: Alcohol (B-spline)
```{python}
# | label: alcohol-model-bs
# | code-fold: true
def _bs_bounds(col: str) -> tuple[float, float]:
    pooled = pd.concat([df_train[col], holdout_scaled[col]], axis=0).dropna()
...
def _spline_rhs(
    *,
    ...
    df_spline = degree + n_knots
    bs_term = (
        f"bs({x_col}, df={df_spline}, degree={degree}, "
        f"lower_bound={lower_bound:.6f}, upper_bound={upper_bound:.6f})"
    )
    return " + ".join([*base_terms, *extra_terms, bs_term])


base_terms = [str(t) for t in controls]
extra_terms = socio_extra

alcohol_col = str(LECol.ALCOHOL)
alcohol_lb, alcohol_ub = _bs_bounds(alcohol_col)
ALCOHOL_BS_DEGREE = 1
ALCOHOL_BS_KNOTS = 2

rhs_alcohol_bs = _spline_rhs(
    base_terms=base_terms,
    extra_terms=extra_terms,
    x_col=alcohol_col,
    n_knots=ALCOHOL_BS_KNOTS,
    degree=ALCOHOL_BS_DEGREE,
    lower_bound=alcohol_lb,
    upper_bound=alcohol_ub,
)

alc_bs_initial, alc_bs_diag = registry.fit(
    df_train[[LECol.TARGET, *controls, LECol.ALCOHOL]].dropna(),
    name="m_alcohol_bs",
    rhs=rhs_alcohol_bs,
    refit=True,
    cook_distance_threshold=4 / len(df_train),
)
registry.remove("m_alcohol_bs_initial")
display(alc_bs_diag)
```
:::
...
# | label: bmi-model-bs
# | code-fold: true
bmi_col = str(LECol.BMI)
bmi_lb, bmi_ub = _bs_bounds(bmi_col)
BMI_BS_DEGREE = 3
BMI_BS_KNOTS = 2

rhs_bmi_bs = _spline_rhs(
    base_terms=base_terms,
    extra_terms=extra_terms,
    x_col=bmi_col,
    n_knots=BMI_BS_KNOTS,
    degree=BMI_BS_DEGREE,
    lower_bound=bmi_lb,
    upper_bound=bmi_ub,
)

bmi_bs_initial, bmi_bs_diag = registry.fit(
    df_train[[LECol.TARGET, *controls, LECol.BMI]].dropna(),
    name="m_bmi_bs",
    rhs=rhs_bmi_bs,
    refit=True,
    cook_distance_threshold=4 / len(df_train),
)
registry.remove("m_bmi_bs_initial")
display(bmi_bs_diag)
```
:::

```{python}
# | label: tbl-alcohol-bmi-bs-compare
# | tbl-cap: "Alcohol/BMI models (linear vs B-splines), sorted by AIC."
registry.compare(sort_by="aic").drop(columns=["rhs"])[
    ["aic", "rmse", "r2", "adj_r2", "n_obs"]
].loc[["m_alcohol", "m_alcohol_bs", "m_bmi", "m_bmi_bs"]].sort_values("aic").round(3)
```

```{python}
# | label: tbl-alcohol-bmi-bs-holdout
# | tbl-cap: "Holdout performance (2011): linear vs B-spline models."
# | include: false
for name in ["m_alcohol", "m_alcohol_bs", "m_bmi", "m_bmi_bs"]:
    _ = registry.evaluate_on(
        name=name,
        df=holdout_scaled,
        label=f"year{HOLDOUT_YEAR}",
    )
...
registry.compare(sort_by=holdout_rmse_col).drop(columns=["rhs"])[
    ["aic", "aicc", "rmse", holdout_rmse_col, holdout_r2_col, "n_obs"]
].loc[["m_alcohol", "m_alcohol_bs", "m_bmi", "m_bmi_bs"]].round(3)
```

```{python}
# | label: fig-alcohol-bmi-spline-curves
# | fig-cap: "Observed LOWESS curve vs LOWESS-smoothed fitted values (linear vs B-spline). The fitted curves smooth model predictions at the observed covariate patterns, which makes them directly comparable to the bivariate LOWESS curve."
# | echo: false
import statsmodels.api as sm
from typing import Any


def _plot_fit_lowess(
    *,
    ax: plt.Axes,
    x_col: str,
    linear_model: Any,
    ...
    data = getattr(getattr(spline_model, "model", None), "data", None)
    frame = getattr(data, "frame", None)
    if not isinstance(frame, pd.DataFrame):
        raise ValueError(
            "Expected a formula-fitted statsmodels model with a training DataFrame."
        )

    target = str(getattr(data, "ynames", LECol.TARGET))
    cols = [c for c in frame.columns if c != target]
    used = df_ref[[target, *cols]].dropna()

    x = used[x_col].astype(float)
    ...
    obs_curve = sm.nonparametric.lowess(y, x, frac=frac, it=0, return_sorted=True)
    ax.plot(
        obs_curve[:, 0],
        obs_curve[:, 1],
        color="tab:blue",
        linestyle=":",
        linewidth=2.0,
        label="LOWESS (observed)",
    )

    pred_lin = pd.Series(linear_model.predict(used), index=used.index)
    pred_spl = pd.Series(spline_model.predict(used), index=used.index)

    lin_curve = sm.nonparametric.lowess(
        pred_lin, x, frac=frac, it=0, return_sorted=True
    )
    spl_curve = sm.nonparametric.lowess(
        pred_spl, x, frac=frac, it=0, return_sorted=True
    )
    ...
    ax.plot(
        ...
        color="tab:red",
        linewidth=2.5,
        label="B-spline (fitted)",
    )

    ax.set_title(title)
    ax.set_xlabel(scaled_train_ds.get_pretty_name(x_col))
    ax.set_ylabel(scaled_train_ds.get_pretty_name(str(LECol.TARGET)))
    ax.legend()
...
_plot_fit_lowess(
    ax=axes[0],
    x_col=str(LECol.ALCOHOL),
    linear_model=registry.get("m_alcohol").diag.model,
    spline_model=registry.get("m_alcohol_bs").diag.model,
    df_ref=df_train,
    title="Alcohol: fitted trend vs LOWESS",
)
_plot_fit_lowess(
    ax=axes[1],
    x_col=str(LECol.BMI),
    linear_model=registry.get("m_bmi").diag.model,
    spline_model=registry.get("m_bmi_bs").diag.model,
    df_ref=df_train,
    title="BMI: fitted trend vs LOWESS",
)
plt.tight_layout()
plt.show()
```

In @fig-alcohol-bmi-spline-curves, the spline fits track the bivariate LOWESS trend closely. In-sample AIC improves for alcohol (spline vs linear) but not for BMI, so we treat the BMI nonlinearity as descriptive rather than as a strong improvement in predictive accuracy.

Influence diagnostics indicate that a small set of countries has disproportionate impact on the fit, so alcohol/BMI conclusions should be framed as descriptive associations and checked for stability under sensitivity analyses.

## Effect modification (interactions)

**RQ5:** test whether the alcohol association differs by development status (effect modification).

Base model: `life_expectancy ~ status_developed + economic_development_PC1 + social_development_PC1 + undp_socio_PC1 + I(undp_socio_PC1^2) + hiv_aids + alcohol`
Interaction model: `life_expectancy ~ (...) + status_developed:alcohol`

```{python}
# | label: interaction-models
# | code-fold: true

main_terms = [*controls, LECol.HIV_AIDS, LECol.ALCOHOL]
...
int_diag = fit_additive(
    ...
    terms=main_terms,
    extra_terms=[*socio_extra, f"{STATUS_DUMMY}:{LECol.ALCOHOL}"],
    refit=True,
)
display(int_diag)
```

<!--  TODO: plot a matrix: ALCOHOL against all control features  -->
```{python}
...
target_pretty = scaled_train_ds.get_pretty_name(str(LECol.TARGET))
ax.set_xlabel(scaled_train_ds.get_pretty_name(str(LECol.ALCOHOL)))
ax.set_ylabel(f"Predicted {target_pretty}")
plt.show()
```
...
```{python}
# | label: tbl-alcohol-interaction-terms
# | tbl-cap: "Key coefficients for the alcohol interaction model (main effect + slope difference by status)."
model = int_diag.model
params = model.params
pvals = model.pvalues
ci = model.conf_int().rename(columns={0: "ci_low", 1: "ci_high"})

alcohol_term = str(LECol.ALCOHOL)
interaction_term = f"{STATUS_DUMMY}:{LECol.ALCOHOL}"
...
coef_tbl = (
    ...
    .join(ci)
    .loc[[alcohol_term, interaction_term]]
    .round(3)
)
display(coef_tbl)

beta_alc = float(params[alcohol_term])
beta_int = float(params[interaction_term])
pd.DataFrame(
    {
        "alcohol_slope": {
            "developing (status_developed=0)": beta_alc,
            "developed (status_developed=1)": beta_alc + beta_int,
            "difference (developed - developing)": beta_int,
        }
    }
).round(3)
```

The interaction plot in @fig-interaction-slopes indicates that the **estimated alcohol slope differs by development status**, with the developed-country slope shifted relative to the developing baseline (see @tbl-alcohol-interaction-terms for the implied slopes).

### Exploratory interaction search (child health and disease burden)

As an additional exploratory check, we test whether the association of disease burden differs systematically with child-health conditions. We focus on four predictors: `child_mortality_PC1` (infant and under-5 deaths), `child_nutrition_PC1` (thinness), `hiv_aids`, and `measles`. Using the same socioeconomic baseline as in RQ2, we run a small interaction “grid search” (one interaction at a time) and then a stepwise selection using AICc as the criterion. We consider the following candidate interactions:

```{python}
# | label: interaction-health-grid-1
# | include: false
child_mortality_pc1 = pcs_by_group.get("child_mortality", [None])[0]
child_thinness_pc1 = pcs_by_group.get("child_nutrition", [None])[0]
if child_mortality_pc1 is None or child_thinness_pc1 is None:
    raise ValueError(
        "Expected child-health PCs missing; check the grouped PCA block (`feature_groups`)."
    )
```

```{python}
# | label: interaction-health-grid-2
# | code-fold: true
health_terms = [child_mortality_pc1, child_thinness_pc1, str(LECol.HIV_AIDS), str(LECol.MEASLES)]
base_terms = [*controls, *health_terms]

candidate_interactions = [
    f"{STATUS_DUMMY}:{child_mortality_pc1}",
    f"{STATUS_DUMMY}:{child_thinness_pc1}",
    f"{STATUS_DUMMY}:{LECol.HIV_AIDS}",
    f"{STATUS_DUMMY}:{LECol.MEASLES}",
    f"{child_mortality_pc1}:{child_thinness_pc1}",
    f"{child_mortality_pc1}:{LECol.HIV_AIDS}",
    f"{child_thinness_pc1}:{LECol.HIV_AIDS}",
    f"{child_mortality_pc1}:{LECol.MEASLES}",
    f"{child_thinness_pc1}:{LECol.MEASLES}",
    f"{LECol.HIV_AIDS}:{LECol.MEASLES}",
]
```
```{python}
# | label: interaction-health-grid-3
# | include: false

base_idx = registry.get("m_socio_eco_social_cook").diag.design_matrix.index
df_health = df_train.loc[
    base_idx,
    [LECol.TARGET, *controls, *health_terms],
].dropna()

rhs_base = " + ".join(base_terms)

tmp = ModelRegistry(eval_year=HOLDOUT_YEAR)
tmp.fit(df_health, name="tmp_health_additive", rhs=rhs_base, refit=True)
tmp.evaluate_on(holdout_scaled, name="tmp_health_additive", label=f"year{HOLDOUT_YEAR}")
base_holdout = tmp.get("tmp_health_additive").eval_metrics_by_label[f"year{HOLDOUT_YEAR}"]

rows: list[dict[str, float | str]] = []
for term in candidate_interactions:
    name = f"tmp_int_{term.replace(':', '_')}"
    rhs = f"{rhs_base} + {term}"
    diag = tmp.fit(df_health, name=name, rhs=rhs, refit=True)
    tmp.evaluate_on(holdout_scaled, name=name, label=f"year{HOLDOUT_YEAR}")
    holdout = tmp.get(name).eval_metrics_by_label[f"year{HOLDOUT_YEAR}"]
    rows.append(
        dict(
            interaction=term,
            aicc=float(diag.metrics.aicc) if diag.metrics.aicc is not None else float("nan"),
            aic=float(diag.metrics.aic) if diag.metrics.aic is not None else float("nan"),
            rmse_train=float(diag.metrics.rmse),
            rmse_holdout=float(holdout.rmse),
            delta_rmse_holdout=float(holdout.rmse - base_holdout.rmse),
        )
    )

pd.DataFrame(rows).sort_values(["rmse_holdout", "aicc"]).head(10).round(3)
```

```{python}
# | label: interaction-health-stepwise
# | code-fold: true
health_add = registry.fit(
    df_health,
    name="m_health_burden_additive",
    rhs=rhs_base,
    refit=True,
)

health_int = registry.fit_stepwise(
    df_health,
    name="m_health_burden_interaction",
    base_terms=base_terms,
    candidates=candidate_interactions,
    direction="stepwise",
    criterion="aicc",
    threshold=1.0,
    refit=True,
...
path = registry.get("m_health_burden_interaction").selection_path
display(path.summary_table().round(3))

for name in ["m_health_burden_additive", "m_health_burden_interaction"]:
    _ = registry.evaluate_on(name=name, df=holdout_scaled, label=f"year{HOLDOUT_YEAR}")
...
```{python}
# | label: tbl-health-interaction-compare
# | tbl-cap: "Additive vs interaction health-burden models (AICc + holdout performance)."
holdout_rmse_col = f"year{HOLDOUT_YEAR}_rmse"
holdout_r2_col = f"year{HOLDOUT_YEAR}_r2"
registry.compare(sort_by="aicc").drop(columns=["rhs"])[
    ["aicc", "rmse", holdout_rmse_col, holdout_r2_col, "n_obs"]
].loc[["m_health_burden_additive", "m_health_burden_interaction"]].round(3)
```

```{python}
# | label: tbl-health-interaction-coefs
# | tbl-cap: "Selected interaction terms (coefficients and p-values) in the stepwise health-burden model."
# | code-fold: true
model = registry.get("m_health_burden_interaction").diag.model
params = model.params
pvals = model.pvalues
ci = model.conf_int().rename(columns={0: "ci_low", 1: "ci_high"})
...
coef_tbl = (
    ...
    .join(ci)
    .loc[[idx for idx in params.index if ":" in idx]]
    .round(3)
)
coef_tbl
```
...
```{python}
# | label: fig-interaction-health
# | fig-cap: "Interaction effect plots for the stepwise-selected health-burden model (up to two selected interactions)."
# | code-fold: true
from ama_tlbx.plotting.regression_plots import plot_interaction_effect

rhs_terms = registry.get("m_health_burden_interaction").rhs.split(" + ")
selected = [t for t in rhs_terms if ":" in t]
if not selected:
    print("No interaction selected by AICc stepwise search (model remains additive).")
else:

    def _choose_x_by(left: str, right: str) -> tuple[str, str]:
        if STATUS_DUMMY in {left, right}:
            return (right if left == STATUS_DUMMY else left), STATUS_DUMMY
        if str(LECol.HIV_AIDS) in {left, right}:
            return str(LECol.HIV_AIDS), (right if left == str(LECol.HIV_AIDS) else left)
        if str(LECol.MEASLES) in {left, right}:
            return str(LECol.MEASLES), (right if left == str(LECol.MEASLES) else left)
        return left, right

    to_plot = selected[:2]
    fig, axes = plt.subplots(1, len(to_plot), figsize=(8 * len(to_plot), 4))
    axes_arr = np.atleast_1d(axes)
    for ax, interaction in zip(axes_arr, to_plot, strict=False):
        left, right = interaction.split(":", maxsplit=1)
        x_col, by_col = _choose_x_by(left, right)
        plot_interaction_effect(
            registry.get("m_health_burden_interaction").diag,
            x=x_col,
            by=by_col,
            df=df_health,
            ax=ax,
...
Stepwise selection adds two interactions (see @tbl-health-interaction-coefs): (i) `status_developed:child_nutrition_PC1`, which implies that the **child-nutrition axis has a different slope** in developed vs developing countries, and (ii) `child_mortality_PC1:hiv_aids`, a weaker term that suggests the **HIV association changes with child-mortality conditions**.
...
available_models = [m for m in candidate_models if m in compare_tbl.index]
holdout_rmse_col = f"year{HOLDOUT_YEAR}_rmse"
best_name = (
    compare_tbl.loc[available_models, holdout_rmse_col].dropna().idxmin()
    if available_models
...
plt.figure(figsize=(7, 5))
ax = metrics.plot_calibration(bins=10, bootstrap=300, random_state=42)
target_pretty = scaled_train_ds.get_pretty_name(str(LECol.TARGET))
ax.set_xlabel(f"Predicted {target_pretty}")
ax.set_ylabel(f"Observed {target_pretty}")
plt.show()
...
cmp_tbl(panel_registry, ["m_panel"])
```
...
- **RQ2 (socioeconomic-only prediction):** a socioeconomic baseline already captures a large share of the life expectancy gradient (see @tbl-socioeconomic-compare). The best AIC baseline after Cook’s-distance sensitivity trimming is `m_socio_eco_social_cook` (AIC **776.5**, RMSE **2.81 years** on the 2014 training cross-section); its 2011 calibration/performance is summarized in @fig-socioeconomic-calibration and @tbl-socioeconomic-eval.
...
- **RQ4 (alcohol/BMI conditional effects):** BMI shows a nonlinear positive association after socioeconomic controls (increase then saturation), while alcohol shows only mild nonlinearity (mostly flat with an upper-tail increase).
