...
<!-- TODO: Does it make sense to use adult mortality as predictive feature? -->
```{python}
# | label: data-prep
# | code-fold: true
...
hdr_hdi = (
    ...
    .df[[HDRCol.ISO3, HDRCol.YEAR, HDRCol.HDI]]
...
pooled_ds = LifeExpectancyDataset(df=pooled)
pooled_norm = pooled_ds.tf_and_norm()

train_raw = pooled_norm.query("year == @TRAIN_YEAR").set_index(LECol.COUNTRY).drop(columns=[LECol.YEAR])
holdout_raw = pooled_norm.query("year == @HOLDOUT_YEAR").set_index(LECol.COUNTRY).drop(columns=[LECol.YEAR])

# Ensure a consistent status dummy (if not already present)
if STATUS_DUMMY not in train_raw.columns:
    if LECol.STATUS in train_raw.columns:
        train_raw = train_raw.assign(
            **{STATUS_DUMMY: train_raw[LECol.STATUS].astype(int)}
        )
        holdout_raw = holdout_raw.assign(
            **{STATUS_DUMMY: holdout_raw[LECol.STATUS].astype(int)}
        )

# tf_and_norm already standardizes numeric predictors (status/year excluded)
train_scaled = train_raw.copy()
holdout_scaled = holdout_raw.copy()

# Keep full feature set for supervised PCA
train_full = train_scaled.copy()
holdout_full = holdout_scaled.copy()
```
...
train_scaled = pca_groups.reduced_df.assign(**train_full.drop(columns=reduced_columns))
holdout_scaled = pca_groups.transform(holdout_full).assign(
    **holdout_full.drop(columns=reduced_columns)
)
...
m1 = registry.fit(
    train_scaled[[LECol.TARGET, LECol.ADULT_MORTALITY]].dropna(),
...
m1_hdi = registry.fit(
    train_scaled[[LECol.TARGET, LECol.ADULT_MORTALITY, LECol.HDI]].dropna(),
...
train_base = train_scaled[[LECol.TARGET, *base_block]].dropna()
...
immun_reduced = registry.fit(
    train_scaled[immun_cols].dropna(),
...
train_alc = train_scaled[model_cols].dropna()
...
nl_df = train_scaled[[LECol.TARGET, *nl_base]].dropna()
...
train_int = train_scaled[[LECol.TARGET, *main_terms]].dropna()
