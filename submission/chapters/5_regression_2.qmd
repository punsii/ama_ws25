...
`life_expectancy ~ status_developed + economic_development_PC1 + social_development_PC1 + undp_socio_PC1 + I(undp_socio_PC1^2)` ($n=156$).
...
All coefficients are **conditional associations** (not causal effects). Developed status remains substantial (**+4.33 years**, $t=5.06$, $p<0.001$) even after the continuous development PCs. The economic-development component is positive (**+1.10 years per 1 SD**, $t=3.84$, $p<0.001$). The UNDP socioeconomic component dominates the fit and is nonlinear: the linear term is large (**-2.53**, $t=-9.41$, $p<0.001$) and the quadratic term is also negative (**-0.53**, $t=-8.70$, $p<0.001$), implying concavity (non-constant marginal association). The `social_development_PC1` effect is small (**-0.53**, $p=0.013$) and is plausibly sensitive to overlap and suppression between multiple correlated “development” summaries (moderate collinearity; max VIF $\approx 6.17$).

To interpret the quadratic UNDP term, let $x$ denote the UNDP PC1 score. The fitted contribution is $\beta_1 x + \beta_2 x^2$, so the marginal association is
$$
\frac{\partial \hat{y}}{\partial x} = \beta_1 + 2\beta_2 x \approx -2.53 - 1.05x.
$$
Because PCA signs are arbitrary, interpretation should be stated via the loadings; equivalently, one may flip a PC’s sign to align “more development” with increasing predicted life expectancy (predictions unchanged), yielding a narrative of increasing life expectancy with diminishing returns.
...
Immunization and HIV/AIDS model: `life_expectancy ~ m_socio_eco_social_cook + immunization_PC1 + hiv_aids`
...
```{python}
# | label: tbl-alc-bmi-scatter
# | fig-cap: "Scatter plots of life expectancy against alcohol consumption and BMI (with linear fit lines)."
...
- **RQ2 (socioeconomic-only prediction):** a socioeconomic baseline already captures a large share of the life expectancy gradient (see @tbl-socioeconomic-compare). The best AIC baseline after Cook’s-distance sensitivity trimming is `m_socio_eco_social_cook` (AIC **776.5**, RMSE **2.81 years** on the 2014 training cross-section); its 2011 calibration/uncertainty is summarized in @fig-socioeconomic-calibration and @tbl-socioeconomic-eval-ci.
