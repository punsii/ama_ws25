...
## Raw panel consistency checks (minimal diagnostics)

The Kaggle comments claim that the raw panel has inconsistent units and significant missingness. Below are minimal, reproducible checks on the **raw panel** (`aggregate_by_country=False`, no imputation) that surface those issues directly.
...
The raw panel’s missingness is concentrated in a small subset of predictors (notably population, HepB coverage, and GDP per capita), which explains the sharp drop in complete‑case sample size.
...
(
    ...
    .agg(
        max_val=("value", "max"),
        p99=("value", lambda s: s.quantile(0.99)),
        share_gt_1000=("value", lambda s: (s > 1000).mean() * 100),
    )
    .reset_index()
)
```

The magnitudes above show that `measles`, `infant_deaths`, and `under_five_deaths` behave like **counts**, not per‑1,000 rates. This is consistent with the extremely large maxima (e.g., measles > 200,000).
...
Nearly half of the rows exceed 100 in `percentage_expenditure`, indicating that it should be interpreted as a scaled spending measure rather than a literal percentage.
...
Population values range from double‑digit counts to 1.3B. The non‑trivial share below 100k suggests mixed scaling or reporting gaps, motivating transformation and sensitivity checks downstream.


## Verifying specific Kaggle claims (data-only)

The comments quoted above include concrete numeric claims. We verify those directly from the raw panel below.

```{python}
# | label: tbl-claim-extremes
# | tbl-cap: "Extremes reported in Kaggle comments, verified from the raw panel."
(
    panel.agg(
        min_population=(LECol.POPULATION, "min"),
        max_population=(LECol.POPULATION, "max"),
        max_infant_deaths=(LECol.INFANT_DEATHS, "max"),
        max_under_five_deaths=(LECol.UNDER_FIVE_DEATHS, "max"),
        max_measles=(LECol.MEASLES, "max"),
        max_bmi=(LECol.BMI, "max"),
        max_percentage_expenditure=(LECol.PERCENTAGE_EXPENDITURE, "max"),
        min_gdp=(LECol.GDP, "min"),
        max_gdp=(LECol.GDP, "max"),
    )
    .stack()
    .rename("Value")
    .reset_index()
...
```{python}
# | label: tbl-population-tiny
# | tbl-cap: "Examples of very small population values (raw panel)."
(
    panel.assign(year=lambda d: d[LECol.YEAR].dt.year)
    .loc[:, [LECol.COUNTRY, "year", LECol.POPULATION]]
    .dropna()
    .sort_values(LECol.POPULATION)
    .head(12)
)
```
...
```{python}
# | label: tbl-country-claims
# | tbl-cap: "Country-specific examples cited in Kaggle comments (infant deaths summary)."
(
    panel.assign(year=lambda d: d[LECol.YEAR].dt.year)
    .query("country in ['India', 'Switzerland', 'Sweden', 'Uzbekistan']")
    .groupby(LECol.COUNTRY)
    .agg(
        min_infant_deaths=(LECol.INFANT_DEATHS, "min"),
        max_infant_deaths=(LECol.INFANT_DEATHS, "max"),
    )
    .reset_index()
)
...
```{python}
# | label: tbl-uzbekistan-2013-2015
# | tbl-cap: "Uzbekistan infant deaths in 2013–2015 (raw panel)."
(
    panel.assign(year=lambda d: d[LECol.YEAR].dt.year)
    .query("country == 'Uzbekistan' and 2013 <= year <= 2015")
    .loc[:, [LECol.COUNTRY, "year", LECol.INFANT_DEATHS]]
    .dropna()
    .sort_values("year")
)
```

Tables @tbl-claim-extremes through @tbl-uzbekistan-2013-2015 confirm the *numeric* inconsistencies raised by Kaggle users: extreme maxima in measles and child‑death columns (far above 1,000), BMI values above 60, population entries as low as double‑digit counts, and development‑status labels that place France/Finland in the developing category. These checks are strictly data‑internal; they do not rely on external sources.

**Implication for preprocessing.** The evidence above indicates that several variables are on inconsistent or ambiguous scales (counts vs per‑1,000 rates; percent vs scaled spending), and that missingness is concentrated in a few high‑impact predictors. This motivates (i) explicit unit labeling and transformations, (ii) robust outlier handling, and (iii) sensitivity checks that report how complete‑case filtering changes sample size.
