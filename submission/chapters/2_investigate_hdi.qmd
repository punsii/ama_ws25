...
This matters for two reasons namely, because HDI as defined in @UNDP_HDR2025_TechNotes is mechanistically related to life expectancy, so treating HDI as a predictor of life expectancy would induce target leakage. Furthermore, correct identification of the feature is mandatory for labeling and interpretation.

Our verification strategy is a direct identity check on a shared key: @UNDP_HDR25_CompositeTimeSeries provides [HDI time series](https://hdr.undp.org/data-center/documentation-and-downloads) by ISO3 country code from 2000 to 2021, overlapping with the Kaggle panel (2000–2015). We map Kaggle countries to ISO3 country codes, merge with UNDP HDR data, select the overlapping years, and quantify agreement (correlation, regression slope/intercept, and error distribution).
...
We reshape the UNDP HDR data from wide to long format, filter to the overlapping years (2000–2015), and merge on (ISO3, year). We also map Kaggle country names to ISO3 using the `pycountry` package. Finally, we compute coverage statistics and perform the identity check.
...
The residual distribution in @fig-hdi-diff supports this conclusion: differences are normally distributed and centered slightly below zero, consistent with the small negative intercept and mean bias in Table @tbl-hdi-metrics.
