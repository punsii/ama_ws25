...
```{python}
# | label: supervised-pca-debug
# | include: false
print(f"strong_features: {len(strong_features)}")
print(f"supervised_pcs: {pc_train.shape[1]}")
```
...
## Residual calibration across years with a KAN (pykan)

Cross-year evaluation in @tbl-supervised-pca-eval-all-models indicates that the **Interactions** OLS specification (`spca_interactions`) is the best baseline for temporal generalization among the fitted SPCA variants. To further improve predictive accuracy across years, we fit a **Kolmogorov–Arnold Network (KAN)** @liu2025kankolmogorovarnoldnetworks to the *residuals* of this baseline model using:

- `year_rel = year - 2014` (encoded as a standardized numeric feature), it would have been more optimal to train the model on a year somewhere in the middle of the range, but for simplicity we keep 2014 as the reference point,
- `region` and `country` (one-hot encoded; `country` acts as a country-specific intercept).

This model is a pragmatic **residual calibrator**. Because it uses country identifiers, it improves prediction for *known countries* in the dataset, but it is not designed to generalize to unseen countries.

```{python}
# | label: supervised-pca-kan
# | include: false
import io
...
from ama_tlbx.analysis.ols_helper import evaluate_model

torch.manual_seed(42)
np.random.seed(42)

baseline_model_name = spca_interactions_name
baseline_diag = registry.get(baseline_model_name).diag
...
for year in all_years:
    ...
    em = evaluate_model(baseline_diag, yr_df, label=f"year{year}")
    resid = em.y_true - em.y_pred

    meta = yr_ds.df[["region"]].copy()
    meta["country"] = meta.index
    meta = meta.loc[resid.index]

    frames.append(
        pd.DataFrame(
            {
                "year": year,
                "year_rel": year - TRAIN_YEAR,
                "country": meta["country"],
                "region": meta["region"],
                "y_true": em.y_true,
                "y_pred_baseline": em.y_pred,
                "residual": resid,
            },
        ),
    )

res_df = pd.concat(frames, ignore_index=True).dropna()
...
pre = ColumnTransformer(
    transformers=[
        ...
        ("num", StandardScaler(), ["year_rel"]),
    ],
    remainder="drop",
)
X_train = pre.fit_transform(train_df[["country", "region", "year_rel"]])
X_test = pre.transform(test_df[["country", "region", "year_rel"]])

y_train = train_df["residual"].to_numpy(dtype=np.float32)[:, None]
y_test = test_df["residual"].to_numpy(dtype=np.float32)[:, None]
...
kan = KAN(
    width=[dataset["train_input"].shape[1], 1],
    grid=3,
    k=3,
    seed=42,
    auto_save=False,
...
with redirect_stdout(io.StringIO()), redirect_stderr(io.StringIO()):
    kan.fit(dataset, opt="LBFGS", steps=25, lamb=1e-2)

with torch.no_grad():
    resid_pred = kan(dataset["test_input"]).cpu().numpy()[:, 0]

y_true = test_df["y_true"].to_numpy(dtype=float)
y_pred_baseline = test_df["y_pred_baseline"].to_numpy(dtype=float)
y_pred_kan = y_pred_baseline + resid_pred

```

```{python}
# | label: supervised-pca-kan-train
# | code-fold: true
...
global_perf = pd.DataFrame(
    [
        {
            "model": "Interactions (OLS baseline)",
            "rmse": _rmse(y_true, y_pred_baseline),
            "mae": _mae(y_true, y_pred_baseline),
            "r2": _r2(y_true, y_pred_baseline),
        },
        {
            "model": "Interactions + KAN residual",
            "rmse": _rmse(y_true, y_pred_kan),
            "mae": _mae(y_true, y_pred_kan),
            "r2": _r2(y_true, y_pred_kan),
        },
    ],
).set_index("model")
...
```{python}
# | label: fig-supervised-pca-kan-test
# | fig-cap: "Test-set RMSE across years (global 80/20 split): Interactions baseline vs KAN residual calibration."
rows = []
for year, grp in test_df.assign(y_pred_kan=y_pred_kan).groupby("year"):
    y = grp["y_true"].to_numpy(dtype=float)
    y_base = grp["y_pred_baseline"].to_numpy(dtype=float)
    y_final = grp["y_pred_kan"].to_numpy(dtype=float)
    rows.append({"year": year, "model": "Interactions (OLS baseline)", "rmse": _rmse(y, y_base)})
    rows.append({"year": year, "model": "Interactions + KAN residual", "rmse": _rmse(y, y_final)})

perf_by_year = pd.DataFrame(rows).sort_values(["model", "year"])
plt.figure(figsize=(9, 4.5))
sns.lineplot(data=perf_by_year, x="year", y="rmse", hue="model", marker="o")
plt.axvline(TRAIN_YEAR, color="grey", linestyle="--", linewidth=1, alpha=0.7)
...
### Residual calibration across years (KAN)

If the primary objective is **temporal generalization**, a practical extension is to treat the SPCA-OLS model (trained on a single year) as an interpretable baseline and then fit a flexible learner to the **residuals** across all years. A Kolmogorov–Arnold Network (KAN; `pykan`) can model systematic residual structure as a function of a year offset (e.g., `year_rel = year - 2014`) and optional country/region encodings, yielding a corrected predictor

$$
\hat y = \hat y_{\mathrm{OLS}}^{2014}(\mathrm{PC1}, \ldots, \mathrm{PC10:PC4}) + \hat r_{\mathrm{KAN}}(\Delta\mathrm{year}, \mathbb{1}[\mathrm{country}], \mathbb{1}[\mathrm{region}])
$$.

![OLS baseline + KAN residual calibration (schematic).](../figures/spca_ols_kan_architecture.svg){#fig-spca-ols-kan-architecture width="80%"}
