...
:::{.callout-note collapse="true"}
### Cook's distance trim (initial + filtered)
```{python}
...
Removing high-influence points (Cook's distance > 4/n) yields a sensitivity check on inference. If coefficients and fit metrics stay stable, the original model is not unduly driven by a few influential countries; if they shift materially, robust/trimmed fits should be preferred for inference.
...
Diagnostics indicate generally adequate linear-model behavior with mild departures. Residuals are centered and mostly patternless across PCs, while the residual-vs-fitted curve shows a slight curvature and the scale-location plot suggests modest heteroscedasticity. Normality is borderline: Shapiro-Wilk is acceptable but Jarque-Bera rejects, indicating heavier tails driven by a few outliers. Breusch-Pagan rejects homoscedasticity, whereas White's test does not, pointing to mild, possibly form-dependent heteroscedasticity. Durbin-Watson ~ 2 indicates no material autocorrelation. Several observations show elevated leverage and Cook's distance (notably Syria, Lesotho, and a few high-leverage countries), so coefficient SEs and CIs from the non-robust OLS output should be treated cautiously. For inferential claims, HC3/robust SEs or bootstrap-based intervals would be preferable; predictive metrics remain reliable.
...
Holdout performance remains strong (RMSE ~ 1.86, R^2 ~ 0.95), suggesting limited overfitting despite using many PCs. The bootstrap CIs for RMSE/MAE/R^2 are narrow relative to the point estimates, reinforcing the stability of predictive performance.
