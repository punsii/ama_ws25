...
if screen_rows:
    ...
    # No interactions to test, use empty list
    interaction_candidates = []
...
**Model evolution (concise):** full supervised-PCA (all PCs) -> reduced to significant PCs -> stepwise AIC among PCs -> add top interaction terms -> Cook's-distance trim -> final minor term pruning (m_final).
...
The AIC-ranked model is the best **in-sample** fit; cross-year generalization may favor a simpler specification (see the mean RMSE table below). Use AIC for in-sample explanation and the cross-year mean RMSE/MAE for forecasting across years.
...
Diagnostics for the final model are largely favorable: JB and Shapiro do not reject normality, and residual plots show only mild curvature. The scale-location plot suggests mild heteroscedasticity and BP is borderline (p=0.048), while White's test passes, indicating any non-constant variance is weak and likely form-dependent. Durbin-Watson ~ 2 suggests no autocorrelation. The condition number (~84) is moderate and VIFs are low, so multicollinearity is not a concern. For inference, non-robust SEs/CIs may be slightly optimistic; prefer HC3/robust or bootstrap CIs. For prediction, impacts are minor.
...
Holdout performance is summarized in the table above; the bootstrap CIs quantify uncertainty in those metrics for the fixed model. These CIs reflect evaluation-sample variability, not model-refit uncertainty.
...
Across years, the model with the lowest mean RMSE/MAE is the better choice for generalization, even if it is not the AIC-best in-sample model.
