...
5. **Select parsimonious models** using transparent criteria and search strategies ([Model Selection](4_model_selection.qmd)).
...
Table @tbl-intro-dataset-coverage summarizes the dataset views used downstream. In the 2014 cross-section, carry-forward imputation retains $n=183$ countries; 50 (27.3%) still have at least one missing predictor, so complete-case analyses use $n=133$. Missing-value handling and robustness checks are documented in [Data Cleaning & Preprocessing](2_preprocessing.qmd) and [Outlier Detection](2_outlier_detection.qmd).
...
Developed countries have markedly higher life expectancy in 2014: the mean is 81.1 years (SD 4.2, $n=32$) versus 69.5 years (SD 7.8, $n=151$) for developing countries (Table @tbl-intro-status-stats). Figure @fig-intro-status-gap shows substantial heterogeneity among developing countries and a tighter distribution among developed countries; the two groups overlap mainly in the 74–83 year range. We use development status as a stratifying variable in descriptive plots and as a covariate (and potential effect modifier) in [Regression](5_regression.qmd).
...
The feature–target correlations highlight a broad development gradient: development and human-capital proxies (e.g., HDI and schooling) correlate positively with life expectancy, whereas mortality and disease burden indicators (e.g., adult mortality, HIV/AIDS) correlate negatively (Figure @fig-intro-top-corr). At the same time, several predictors are near-redundant (Table @tbl-intro-collinearity-hotspots), which motivates feature grouping/PCA before interpreting multivariate regression coefficients ([Principal Component Analysis](3_pca.qmd), [Regression](5_regression.qmd)). Because HDI includes life expectancy by construction, its correlation with the target is partly mechanical; we therefore treat it primarily as a descriptive proxy and assess sensitivity in [Investigate HDI](2_investigate_hdi.qmd).
