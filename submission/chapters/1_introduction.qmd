...
<!-- TODO: we need to start with a more general overview of the dataset views, and not jus thte coverage as below, what else might we include here? -->
...
cross_section_missing.style.hide(axis="index")
```
...
predictor_stats_2014_by_group["Demography & mortality"].round(2).rename_axis("Feature").style.hide(axis="index")
```
...
predictor_stats_panel_by_group["Demography & mortality"].round(2).rename_axis("Feature").style.hide(axis="index")
```
...
predictor_stats_2014_by_group["Immunization & infectious disease burden"].round(2).rename_axis("Feature").style.hide(axis="index")
```
...
predictor_stats_panel_by_group["Immunization & infectious disease burden"].round(2).rename_axis("Feature").style.hide(axis="index")
```
...
group_cols = [
    col
    for col in predictor_groups["Immunization & infectious disease burden"]
    if col != LECol.STATUS
]
n_features = len(group_cols)
fig, axes = plt.subplots(
...
predictor_stats_2014_by_group["Socio-economic & investment"].round(2).rename_axis("Feature").style.hide(axis="index")
```
...
predictor_stats_panel_by_group["Socio-economic & investment"].round(2).rename_axis("Feature").style.hide(axis="index")
```
...
predictor_stats_2014_by_group["Nutrition & development status"].round(2).rename_axis("Feature").style.hide(axis="index")
```

```{python}
# | label: tbl-intro-predictor-nutrition-panel
# | tbl-cap: "Nutrition summary statistics (2000–2015 panel; numeric features)."
predictor_stats_panel_by_group["Nutrition & development status"].round(2).rename_axis("Feature").style.hide(axis="index")
```
