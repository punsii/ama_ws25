...
dataset_latest = LifeExpectancyDataset.from_csv_updated(
    resolve_nand_pred="carry_forward",
...
Based on our OLS Regression result we get an `R-Squared: 0.981` Which means that the model explains 99% of the variance of the dataset. `Adj. R-Squared: 0.979` Shows that if you adjust for the number of features the amount still remains high. Showing that the majority of features play a signficant part of the analysis.
...
The most notable outliers are **Barbados**, **South Africa**, and **Sierra Leone**. Barbados shows high studentized residuals (around 2-3) with moderate leverage, while South Africa and Sierra Leone appear as high-leverage points. However, most countries including **Lesotho**, **Angola**, **Botswana**, **Eswatini**, and **India** (which show as large circles indicating high Cook's distance) remain within acceptable bounds (`|y| < 2`), suggesting the model is generally robust despite these influential points.

### Predictive capabilities

To test some predictive capabilities we decided to perform some single variable analyses, here we chose `Schooling` because it showed the strongest univariate relationship with life expectancy (R² = 0.553) among features that don't suffer from data leakage.
...
The 2011 model achieves `R² = 0.983` on its own test data, demonstrating exceptionally strong predictive power for that year. The 2014 model, trained on data three years later, shows `R² = 0.970` on its test set—also an excellent result, explaining 97% of the variance, though slightly lower than the 2011 model. This difference could reflect changes in data quality, sample size, or the relationships between variables over time.
...
- **Old → Old test** (R² = 0.983): Exceptionally strong baseline performance when the 2011 model predicts 2011 data
- **Latest → Latest test** (R² = 0.970): Excellent performance when the 2014 model predicts 2014 data, though slightly lower than the 2011 baseline
- **Old → Latest test** (R² = 0.952): Strong cross-temporal prediction—the 2011 model generalizes well to 2014 data

The third scenario is particularly noteworthy: the 2011 model achieves R² = 0.952 when predicting 2014 data, which is lower than the 2014 model's performance on its own test set (0.970) but still demonstrates excellent generalization. This suggests that the relationships between health, economic, and social factors learned from 2011 data generalized remarkably well to 2014, indicating relatively stable predictive patterns over this three-year period. The 2014 model's superior performance on its own data is expected, as it captures the specific relationships in that year's data.
...
**Strong Predictors (R² > 0.4):**
- **Schooling (R² = 0.553)**: The strongest valid single predictor, showing a clear positive relationship. Education consistently correlates with better health outcomes, as countries with higher average years of schooling tend to have longer life expectancies.
- **HIV/AIDS (R² = 0.431)**: Strong negative correlation—higher HIV/AIDS prevalence significantly reduces life expectancy.

**Strong Predictors (continued):**
- **GDP (R² = 0.656)**: Economic output shows strong predictive power, reflecting that wealthier nations have better healthcare access and infrastructure.

**Moderate Predictors (0.1 < R² < 0.5):**
- **BMI (R² = 0.307)**: Shows a positive relationship with life expectancy. Higher average BMI typically indicates better nutrition and food security.
- **Percentage Expenditure (R² = 0.139)**: Healthcare spending as a percentage of GDP shows moderate predictive power.
- **Alcohol (R² = 0.140)**: Shows a positive relationship, possibly because wealthier nations with better healthcare also have higher alcohol consumption, creating a confounding effect.

**Strong Predictors (continued):**
- **Baby Deaths (R² = 0.847)**: Extremely strong negative correlation—infant and child mortality is one of the strongest individual predictors of life expectancy.

**Moderate-to-Weak Predictors:**
- **Measles (R² = 0.216)**: Shows moderate negative correlation with life expectancy.
- **Child Thinness (R² = 0.199)**: Negative correlation as expected—higher malnutrition rates are associated with lower life expectancy.
- **Immunisation (R² = 0.297)**: Shows meaningful predictive power, indicating vaccination coverage correlates with life expectancy outcomes.
- **Total Expenditure (R² = 0.007)**: Essentially no linear relationship, suggesting health spending alone doesn't predict outcomes—how funds are allocated matters more than the amount spent.
- **Population (R² = 0.001)**: No meaningful linear relationship with life expectancy.
...
ax.set_xlabel('Alcohol Consumption (SD from mean)', fontsize=12)
...
**Key Findings:**
- **Developed Countries (Blue, R² = 0.212)**: The regression line shows a negative slope, indicating that among developed nations, higher alcohol consumption is associated with slightly lower life expectancy. However, the relatively low R² indicates this relationship is weak. Developed countries cluster at high life expectancy (mostly 76-82 years) with relatively narrow variation despite differences in alcohol consumption.

- **Developing Countries (Orange, R² = 0.015)**: Shows virtually no correlation, with an extremely low R². The much wider spread in life expectancy (50-80 years) across developing nations suggests that factors other than alcohol consumption are far more influential in determining outcomes. The normalized alcohol consumption shows variability, yet this explains less than 2% of life expectancy variance.

**Interpretation:**

When we examine the overall positive correlation (R² = 0.140) observed in the normalized combined data, separating by development status reveals dramatically different patterns. The apparent correlation exists primarily because:

1. **Developed countries** (blue points at top) show a negative relationship (R² = 0.212) where higher alcohol consumption is associated with lower life expectancy, though the effect is modest
2. **Developing countries** (orange points) span a wider range of both alcohol consumption and life expectancy, but show virtually no relationship (R² = 0.015), indicating alcohol is not a meaningful predictor in this group
3. The overall positive correlation in combined data is driven by the **between-group differences** (developed vs developing) rather than consistent **within-group relationships** between alcohol and longevity
...
The multivariate model using normalized features achieves **R² = 0.9528**, which represents a dramatic improvement over the best individual predictor (Child Thinness with R² = 0.599). This demonstrates the powerful synergistic effect of combining multiple features:

**Key Insights:**
1. **Synergistic Effects**: Features that showed weak individual predictive power (like Total Expenditure, R² = 0.007) contribute meaningfully when combined with other variables, capturing interaction effects that aren't visible in isolation.

2. **Feature Coefficients**: The bar chart reveals the strongest predictors after normalization:
   - **Baby Deaths (-5.26)**: Strongest negative impact—infant mortality dramatically reduces life expectancy
   - **Gdp (+2.77)**: Strong positive effect, reflecting economic development's role in health outcomes
   - **Hiv Aids (-2.64)**: Strong negative impact—diseases dramatically reduce life expectancy
   - **Bmi (-0.62)**: Negative coefficient, suggesting complex relationship
   - **Schooling (-0.57)**: Negative coefficient in multivariate context (differs from univariate)
   - **Total Expenditure (+0.57)**: Positive effect when combined with other features
   - **Measles (-0.19)**: Small negative impact
   - **Percentage Expenditure (+0.18)**: Healthcare spending shows positive effect
   - **Alcohol (+0.10)**: Small positive coefficient
   - **Child Thinness (-0.06)**, **Immunisation (+0.04)**, **Population (+0.03)**: Minimal coefficients

3. **Normalization Impact**: The normalized model (R² = 0.9528) shows exceptional predictive power, as standardization allows fair comparison across features with different scales. The coefficient magnitudes now reflect standardized units, making them directly comparable.

The exceptionally high R² of 95% indicates that these 12 features capture nearly all the systematic variation in life expectancy, with only 5% remaining unexplained variance.
...
**Model Performance:**
- The cleaned model achieves **R² = 0.9604** compared to R² = 0.9528 with outliers included
- This represents a **+0.0076 improvement (+0.8%)** in explained variance
...
|---------|----------|------------------|--------|
| **Baby Deaths** | -5.26 | -4.65 | +0.61 (reduced magnitude by 12%) |
| **HIV/AIDS** | -2.64 | -2.86 | -0.22 (increased magnitude by 8%) |
| **GDP** | +2.77 | +2.71 | -0.06 (very stable, -2%) |
| **BMI** | -0.62 | -0.51 | +0.11 (reduced magnitude by 18%) |
| **Schooling** | -0.57 | -0.27 | +0.30 (reduced magnitude by 53%) |
| **Total Expenditure** | +0.57 | +0.23 | -0.34 (reduced by 60%) |
| **Measles** | -0.19 | -0.29 | -0.10 (increased magnitude by 53%) |
| **Percentage Expenditure** | +0.18 | +0.20 | +0.02 (very stable, +11%) |
| **Alcohol** | +0.10 | +0.14 | +0.04 (increased by 40%) |
| **Child Thinness** | -0.06 | -0.46 | -0.40 (increased magnitude by 667%) |
| **Immunisation** | +0.04 | +0.28 | +0.24 (increased by 600%) |
| **Population** | +0.03 | +0.33 | +0.30 (increased by 1000%) |

**Coefficient Changes:**

**Key Findings:**

1. **Improved Predictive Power**: The R² improvement from 0.9528 to 0.9604 indicates that outliers were introducing a small amount of noise. The model now explains 96% of variance in typical countries, up from 95%, demonstrating already excellent performance.

2. **Stable Core Predictors**: 
   - **GDP (+2.77 → +2.71, -2% change)**: Extremely stable, confirming economic development is consistently important
   - **HIV/AIDS (-2.64 → -2.86, +8% magnitude increase)**: Slightly stronger effect without outliers
   - **Baby Deaths (-5.26 → -4.65, -12% magnitude decrease)**: Remains the dominant negative predictor, though slightly weaker without outliers
   - **BMI (-0.62 → -0.51, -18%)** and **Percentage Expenditure (+0.18 → +0.20, +11%)**: Relatively stable

3. **Outlier-Sensitive Features** (large changes indicate outliers were heavily influencing these coefficients):
   - **Child Thinness (-0.06 → -0.46, +667% magnitude)**: Dramatically increased importance—outliers were masking its true negative impact on life expectancy
   - **Immunisation (+0.04 → +0.28, +600%)**: Much stronger positive effect without outliers
   - **Population (+0.03 → +0.33, +1000%)**: Became 11x more important when outliers removed
   - **Total Expenditure (+0.57 → +0.23, -60%)**: Reduced substantially—outliers were inflating its apparent importance
   - **Schooling (-0.57 → -0.27, -53% magnitude)**: Effect cut in half without outliers

4. **Practical Implications**: The dramatic changes in Child Thinness, Immunisation, and Population suggest these features have very different relationships in outlier countries compared to typical countries. For policy applications focusing on typical development scenarios, the cleaned model reveals that:
   - Malnutrition (Child Thinness) has much stronger negative impact than the full dataset suggests
   - Vaccination programs (Immunisation) have stronger positive effects than apparent with outliers
   - Population dynamics play a more important role than initially visible
   - The core predictors (GDP, HIV/AIDS, Baby Deaths) remain fundamentally important regardless of outliers
