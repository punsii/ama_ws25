...
# | results: asis
# | echo: false
corr_view = le_xs_2014.analyzer_view(include_target=True, standardized=False)
n_countries = LifeExpectancyDataset.country_count(corr_view.df)
n_rows = int(corr_view.df.shape[0])
display(
    Markdown(
        f"The 2014 cross‑section used for correlations contains {n_countries} countries "
        f"and {n_rows} rows after dropping all rows with **any** missing predictors."
    )
)
```
...
fig_pos, fig_neg = corr_result.plot_target_correlations(n=11)
plt.close(fig_neg)
fig_pos.show()
fig_pos, fig_neg = corr_result.plot_target_correlations(n=8)
fig_neg.show()
plt.close(fig_pos)
```
...
delta_target = (
    ...
    .assign(
        ...
        transform=lambda d: d.index.map(
            lambda x: transform_name(LECol.transform_label(x))
        ),
    )
...
delta_target_corr = delta_target.reset_index()[["feature", "delta"]].rename(
    columns={"delta": "correlation"}
)
delta_target_abs_corr = delta_target.reset_index()[["feature", "delta_abs"]].rename(
    columns={"delta_abs": "correlation"}
)
delta_result = CorrelationResult(
    matrix=corr_result_tf.matrix.loc[common_cols, common_cols]
    - corr_result.matrix.loc[common_cols, common_cols],
    pretty_by_col=corr_result.pretty_by_col,
    feature_pairs=pd.DataFrame(),
    target_correlations=delta_target_corr,
)
delta_result_abs = CorrelationResult(
    ...
    pretty_by_col=corr_result.pretty_by_col,
    feature_pairs=pd.DataFrame(),
    target_correlations=delta_target_abs_corr,
)
...
delta_target[["pretty", "transform", "corr_raw", "corr_tf", "delta", "delta_abs"]].head(
    13
).set_index("pretty")
```
...
fig_pos, fig_neg = delta_result_abs.plot_target_correlations(n=13)
plt.close(fig_neg)
fig_pos.show()
```
...
<!-- TODO: plot final correlation matrix with the reduced feature set and all unaffected columns (after applying transforms) -->
...
summary_pretty = summary_df.assign(
    features=lambda d: d.features.apply(
        lambda s: ", ".join(
            le_xs_2014.get_pretty_name(f.strip()) for f in s.split(",")
        ),
    ),
).loc[:, ["group", "size", "features", "mean_abs_corr", "min_abs_corr"]]
display(summary_pretty.set_index("group"))
```
...
polio_col = str(LECol.POLIO)
target_col = str(LECol.TARGET)
polio_raw = float(base_corr.matrix.loc[polio_col, target_col])
polio_shortfall = float(inv_corr.matrix.loc[f"~{polio_col}", target_col])
polio_tf = float(inv_tf_corr.matrix.loc[polio_col, target_col])

display(
    Markdown(
        f"For Polio coverage, the correlation with life expectancy shifts from "
        f"$r$={polio_raw:.2f} (coverage) to $r$={polio_shortfall:.2f} (shortfall) and "
        f"$r$={polio_tf:.2f} after the default transform, matching the sign flip seen in Figure @fig-coverage-inversion."
    )
)
```
